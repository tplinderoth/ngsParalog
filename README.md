...
and repetitive sequence for example) rely on inflated coverage or excess heterozygosity. We find differences in sequencing depth tend to be less reliable than using signatures of excess
...
approach in which sequencing errors and within-individual read proportions are modeled, thereby offering more power to detect mapping problems at lower FDR.
In addition, the dupHMM.R script implements a Hidden Markov Model to inder start and stop coordinates of mapping problems along large scaffolds based on both sequencing depth
...
Note: Only the calcLR function is currently implemented. Use the dupHMM.R script to calculate start and stop coordinates of regions that having mapping problems for now.

### calcLR

This function will calculate a likelihood ratio (LR) of mismapping reads covering each site. This can be run on any type of enomic NGS data (WGS, RAD-seq, sequence-capture, etc). 
...
only informative sites for the method so it is much more efficient to limit the input to variable sites. Column 5 of the output are the likelihood ratios, which are asymptotically 
...
ngsParalog can take as input a premade mpileup format file. In this first example we will use some simulated mpileup data for 100 SNPs where the first 50 SNPs (118-6197)
have normal mapping, and the following 50 SNPs (6443-8460) are paralogous.
...
to ensure that reads are not truncated based on max depth limits. In the call to ngsParalog it's recommended to use a -minQ of around 20-30.<\b>
...
calculated LRs to the null distribution. The following R code will calculate p-values for the LRs, which can be used to filter the data after adjusting them for the
number of tested sites.
	...
	> lr$pval.adj <- p.adjust(lr$pval, method="bonferroni") # p-values adjust for number of tested sites
	
	# The 7th column of the calcLR output is the adjusted p-value for rejecting the null that reads
	# covering the site derive from a single locus. Of course you can use a p-value adjustment of your
	# choosing, e.g. "fdr".
...
The output hmm_region.rf TSV file lists the regions (chromosome, start position, end position) that are problematic for reading mapping. These
